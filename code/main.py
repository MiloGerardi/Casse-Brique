...
def rebondBrique(brique, bille):
    ...
    rot = -180
    if (bille.getPosition().x >= brique.getPosition().x) and (bille.getPosition().x <= brique.getPosition().x+brique.getTaille().x):
        if (bille.getPosition().y >= brique.getPosition().x) and (bille.getPosition().y <= brique.getPosition().y+brique.getTaille().y):
            print("colision")
